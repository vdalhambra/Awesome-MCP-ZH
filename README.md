...
| [vdalhambra/siteaudit-mcp](https://github.com/vdalhambra/siteaudit-mcp) | 提供 11 种网站审计工具：SEO 评分（meta/heading/canonical/schema）、Lighthouse 性能评测、安全 Headers 检查 (HTTPS/HSTS/CSP/X-Frame)、WCAG 无障碍合规检测、竞品网站对比分析。无需任何 API 密钥。 | 社区实现, Python 开发 🐍, 云服务 ☁️, Google PageSpeed API, 无 API Key 🔑。                                                   |
...
| [vdalhambra/financekit-mcp](https://github.com/vdalhambra/financekit-mcp) | 提供 17 种金融分析工具：实时股票行情、技术指标 (RSI/MACD/布林带/ADX 含信号判断)、加密货币价格、投资组合风险指标 (VaR/夏普/索提诺比率/Beta 值)、期权链及财报日历。无需任何 API 密钥。 | 社区实现, Python 开发 🐍, 云服务 ☁️, yfinance + CoinGecko 数据, 无 API Key 🔑。                                                  |
